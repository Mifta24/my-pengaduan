            ...
            <div class="bg-white shadow-sm ring-1 ring-gray-900/5 rounded-lg">
                ...
                <div class="px-6 py-4 space-y-3">
                    ...
                    <form action="{{ route('admin.categories.destroy', $category) }}" method="POST"
                          onsubmit="return confirm('Apakah Anda yakin ingin menghapus kategori ini? Semua keluhan yang terkait akan kehilangan kategori.')" class="w-full">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                                ...
                                @if($category->complaints->count() > 0) onclick="return confirm('Kategori ini memiliki {{ $category->complaints->count() }} keluhan. Yakin ingin menghapus?')" @endif>
                            <svg class="-ml-0.5 mr-1.5 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
